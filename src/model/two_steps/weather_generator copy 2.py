...
class WeatherGenerator:
    ...
    def fit(self, data, initial_precipitation, initial_weather, initial_date):
        ...
        month_transitions = deepcopy(self.month_transitions)
        mc_states = deepcopy(self.mc_states)
        def _mc_simulate(month, state):
            transition = month_transitions[month]
            state_idx = mc_states.index(state)
            probs = transition[state_idx]
            state = np.random.choice(mc_states, size=1, p=probs)[0]
            return state


        window = deepcopy(self.window)
        data = deepcopy(self.data)
        doy_var = deepcopy(self.doy_var)
        distance_weights = deepcopy(self.distance_weights)
        weather_vars = deepcopy(self.weather_vars)
        def _simulate_knn_weather(weather, state, next_doy, next_state):
            range_size = window
            filter = [False]
            ...
            while not max(filter):
                ...
                # buscando vizinhos
                filter = (
                    data[doy_var].isin(doy_range) &
                    (data['state'] == state) &
                    (data['state'].shift(-1) == next_state)) # next day
            neighbors = data[filter].to_dict('records')
            ...
            distances = [
                np.sqrt(
                    sum(distance_weights[var]*((weather[var] -  neighbor[var])**2)
            ...
            successive_day = data[data['_index'] == successive_day_index].to_dict('records')[0]
            new_weather = {var: successive_day[var] for var in weather_vars}
            return new_weather


        def simulate_weather(date, state, weather):
            ## verificando informações do dia atual
            month = date.month
            state = state
            ## verificando informações do próximo dia
            nex_date = date + datetime.timedelta(days=1)
            next_doy = nex_date.timetuple().tm_yday
            ## simulando estado da precipitação
            new_state = _mc_simulate(month=month, state=state)
            ## selecionando proximo clima
            new_weather = _simulate_knn_weather(
                weather=weather, state=state,
                next_doy=next_doy, next_state=new_state)
            ## atulizando estados da cadeia
            return (nex_date, new_state, new_weather)

        return ((self.date, self.state, self.weather), simulate_weather)
